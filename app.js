...
isAdultPresent = devs.some(devs => (currentYear - devs.year) >= 18);
...
const olderGroup = new Date().getFullYear();
isEveryone19OrOlder = devs.every(devs => (olderGroup - devs.year) >= 19);
...
idx = comments.findIndex(comments => comments.id === 123523);
...
// Used the reduce() method to help calculating the combined lifespan of all inventors
// the callback function calculates the lifespan ofeach inventor. It subtracts their birth year('year') from their year of death ('passed')
// the 2nd argument to 'reduce()' is initial value and is set to 0.
